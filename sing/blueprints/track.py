...
@track_bp.get("/all")
def track_all():
    track_all = db.session.execute(select(Track.id,Track.name,Track.duration)).all()
    # result=[]
    ...
    #     result.append(raw)
    return render_template("track_all.html",tracks=turn_row_into_tuple(track_all))

@track_bp.get("/<track_id>")
def track_one(track_id):
    return render_template("track_all.html",tracks=[db.session.execute(select(Track.id,Track.name,Track.duration).where(Track.id==track_id)).one_or_none()])
